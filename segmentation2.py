...
class DINOv2SegmentationModel(nn.Module):
    ...
    def __init__(self, dinov2_model, num_classes=21, patch_size=14, img_size=224):
        ...
        # Extract dimensions from DINOv2 model
        self.dinov2_dim = dinov2_model.embed_dim
        self.patch_size = patch_size
        self.img_size = img_size
        
        # Set to eval mode as we might want to freeze the encoder
        # self.encoder.eval()
    ...
    def extract_features(self, x):
        ...
        # Get DINOv2 features
        # with torch.no_grad():
        # No need to print shapes in production code
        ...
        # For RGGB input, calculate patches accounting for 4 channels
        num_patches_per_image = num_tokens // batch_size
        h = w = int(np.sqrt(num_patches_per_image))
        # print("H w", h, w, num_patches_per_image)
    ...
    def forward(self, x):
        ...
        # print("Fearures shape: ", features.shape)
    ...
    def prepare_for_distributed_training(self):
        ...
        # Wrap decoder components with FSDP
        # Project layer
...
def get_segmentation_optimizer_scheduler(model, max_iter):
    # Optimizer with more conservative learning rates
    encoder_params = list(model.encoder.parameters())
    decoder_params = list(model.decoder.parameters())
    
    optimizer = torch.optim.AdamW([
        {"params": encoder_params, "lr": 5e-6},  # Lower LR for pretrained encoder
        {"params": decoder_params, "lr": 5e-2}   # Lower LR for decoder
    ], weight_decay=0.01)
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer,
        max_lr=[5e-6, 5e-2],  # Lower max learning rates
...
def make_segmentation_dataset(dataset_str, transform=None, split="train"):
    ...
    class RGGBSegmentationDataset(Dataset):
        def __init__(self, root, split, transform=None, target_transform=None, mean=None, std=None, black_level=0, white_level=1.0):
            ...
            print("PATHS: ", len(self.image_paths), split)
            self.image_paths = self.image_paths[10:11]
            # if split == "train":
            #     pass
            #     # self.image_paths = self.image_paths[]
            # else:
            #     self.image_paths = self.image_paths[700:750]
            # if split == "val":
        ...
        def __getitem__(self, idx):
            ...
            try:
                try:
                    raw_data = self.load_and_trivial_rggb_to_rgb(img_path)
                    # raw_data = np.load(img_path, allow_pickle=True)
                except Exception:
                    print("Img path", img_path)
                    if self.split == "train":
                        raw_data = self.load_and_trivial_rggb_to_rgb(f"/home/paperspace/Documents/nika_space/main_dataset/val/ade/images/{base_name}.npy")
                    else:
                        raw_data = self.load_and_trivial_rggb_to_rgb(f"/home/paperspace/Documents/nika_space/main_dataset/train/ade/images/{base_name}.npy")
        ...
        def load_and_trivial_rggb_to_rgb(self, npy_path):
            ...
            # save_path = "check_standart.png"
            # raw_image.save(save_path)
            # print(f"Saved demosaiced image to {save_path}")
            return raw_image
